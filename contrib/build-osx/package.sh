...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-rby-macos
	cd /tmp/electrum-rby-macos
...
rm -rf /tmp/electrum-rby-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-rby-macos/image/
cp -r $1 /tmp/electrum-rby-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-RBY_uncompressed.dmg \
    /tmp/electrum-rby-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-RBY_uncompressed.dmg electrum-rby-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-RBY_uncompressed.dmg

echo "Done."
md5sum electrum-rby-$VERSION.dmg
