...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-RBY"
  !define PRODUCT_WEB_SITE "https://github.com/pooler/electrum-rby"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-rby-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-rby-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-rby-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-rby-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\rubycoin\shell\open\command" "" "$\"$INSTDIR\electrum-rby-${PRODUCT_VERSION}.exe$\" $\"%1$\""
