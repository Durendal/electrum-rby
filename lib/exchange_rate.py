...
class ExchangeBase(PrintError):
    ...
    def get_currencies(self, history=False):
...
class CoinID(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        result = []
        for curr in list(fiats.keys()):
            result.append((curr, Decimal("%.04f" % fiats['USD']) if curr == 'USD' else Decimal("%.04f" % fiats[curr]) * Decimal("%.04f" % fiats['USD'])))
        return dict(result)

    def history_ccys(self):
        return list(self.fiats)

    def historical_rates(self, ccy):
        rates = self.get_rates(ccy)[ccy]
        res = []
        for i in range(30):
            res.append((datetime.utcfromtimestamp(time.time()-(i*60*60*24)).strftime("%Y-%m-%d"), "%.04f" % rates))
...
class FxThread(ThreadJob):
    ...
    def config_exchange(self):
        return self.config.get('use_exchange', 'CoinID')
    ...
    def set_exchange(self, name):
        class_ = globals().get(name, CoinID)
